...
class Graph(dict):
    ...
    def is_valid_degree(self, degree):
        ...
        """This function returns a boolean telling us whether the specified degree is 
        valid or not The condition for validity is:
    ...
    def is_connected(self):
        ...
        """The purpose of this method is to check whether every node is connected to 
        every other node or not.
    
        The implementation here is the Breadth-First Search. We only need to start the 
        search with one node (just do the first node), and search outwards. If the graph 
        is connected, then all nodes will be connected to that node; if it is not, then 
        at least one node will be absent from the connections."""
        ...
        """Here, the worklist is populated with vertices that are connected to the 
        current vertex. If the vertex is already in the worklist, then it is passed over;
        otherwise, it is added to the worklist."""
        
        for vertex in worklist:
            connected_vs = self.out_vertices(vertex)
            for w in connected_vs:
                if w not in worklist:
                    worklist.append(w)
                elif w in worklist:
                    pass
            
        """Finally, the worklist is compared against the vertices list, iterating 
        through the entire list. If a node is present in the vertices list but absent in
        the worklist, the loop stops, and 'connected' is returned with the boolean value
        of False. Otherwise, 'connected' is True."""
        
        for vertex in vertices:
            if vertex not in worklist:
                connected = False
                break
            else:
                connected = True
